...
class CameraController(
        ...
        future.addListener(
            {
                ...
                try {
                    ...
                    val viewport =
                        view.viewPort ?: run {
                            onError(
                                "Area kamera belum siap. Mulai kembali."
                            )
                            return@addListener
                        }
                    ...
                    previewCase.setSurfaceProvider(
                    ...
                    bound.cameraInfo.cameraState.observe(
                        owner
                    ) { state ->
                        if (state.error != null) {
                            main.post {
                                if (ticket == request) {
                                    onError(
                                        "Kamera mengalami gangguan. Tutup aplikasi kamera lain lalu mulai kembali."
                                    )
                                }
                            }
                        }
                    ...
                    main.postDelayed(
                        {
                            if (
                                ...
                                onError(
                                    "Pratinjau kamera belum tersedia. Mulai kembali."
                                )
                            }
                        },
                        10000L
                    ...
                    onError(
                        "Kamera tidak dapat dibuka: ${e.javaClass.simpleName}"
                    )
                }
            },
    ...
    private fun selector(front: Boolean) =
        if (front) {
