...
class FramePreprocessor(private val config: AppConfig) {
    ...
    fun prepare(
        ...
        for (y in 0 until roi.height) {
            for (x in 0 until roi.width) {
                ...
                val lum = if (config.limitedYuv) {
                    1.16438356f * (luminance[i] - 16f)
                } else {
                    luminance[i]
                }
                ...
                rgb[3 * i] = (
                        lum +
                                (if (config.limitedYuv) 1.5960268f else 1.402f) * v
                        ).coerceIn(0f, 255f)

                rgb[3 * i + 1] = (
                        lum -
                                (if (config.limitedYuv) 0.3917623f else 0.344136f) * u -
                                (if (config.limitedYuv) 0.8129676f else 0.714136f) * v
                        ).coerceIn(0f, 255f)

                rgb[3 * i + 2] = (
                        lum +
                                (if (config.limitedYuv) 2.017232f else 1.772f) * u
                        ).coerceIn(0f, 255f)
            }
        }
    ...
    private fun resizeBilinear(
        ...
        // Pusat piksel half-pixel, batas dijepit, tanpa antialias.
        for (y in 0 until 224) {
            val sy = (
                    (y + 0.5f) * height / 224f - 0.5f
