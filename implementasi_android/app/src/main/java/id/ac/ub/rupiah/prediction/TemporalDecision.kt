...
class TemporalDecision(
    ...
    private fun observe(now: Long) {
        if (
            lastObservation != null &&
            now - lastObservation!! > config.windowMs
        ) {
            window.clear()
            rejectedSince = null
        }
        ...
        while (
            !window.isEmpty() &&
            now - window.first.time > config.windowMs
        ) {
            window.removeFirst()
        }
    }
    ...
    fun accept(
        ...
        for (item in window) {
            for (i in mean.indices) {
                mean[i] +=
                    item.scores[i] / window.size
            }
        }

        val best =
            mean.indices.maxByOrNull {
                mean[it]
            }!!

        if (
            labels[best] == "nonuang" ||
            mean[best] < config.threshold ||
            best != winner
        ) {
            return Result(
                null,
    ...
    fun markAnnounced(label: String) {
        announced = label
    }
}